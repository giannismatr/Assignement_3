...
from fanorona import FanoronaState
from core import Color
from fanorona import FanoronaRules
from fanorona import FanoronaAction
...
class FanoronaGUI(QMainWindow):
    ...
    def __init__(self, app, shape, players, allowed_time=5.0, sleep_time=.500, first_player=-1, boring_limit=50,
                 parent=None):
        super(FanoronaGUI, self).__init__(parent)
        self.app = app
    ...
    def _reset(self):
        ...
        self.state = FanoronaState(board=self.board.get_board_state(), next_player=self.first_player,
    ...
    def step(self, action):
        ...
        """
        assert isinstance(action, FanoronaAction), "action has to be an Action class object"
        result = FanoronaRules.act(self.state, action, self.current_player)
        if isinstance(result, bool):
            return False
        else:
            self.state, self.done = result
            FanoronaRules.moment_player(self.state, self.players)
    ...
    def play_game(self):
        ...
        while not self.done:
            ...
            FanoronaRules.moment_player(state, self.players)
    ...
    def setFleche(self, at, to):
        if at == to:  # EDIT: it is not mandatory to perform a combo
            self.board_gui.squares[at[0]][at[1]].set_div(self.rester)
        elif(to[0]== at[0] + 1 and to[1] == at[1] + 1): #droitehaut
        ...
        elif(to[0]== at[0] - 1 and to[1] == at[1] - 1): #gauchebas
...
if __name__ == "__main__":
    ...
    game = FanoronaGUI((5, 9), agents, sleep_time=sleep_time, allowed_time=allowed_time)
    game.show()
    sys.exit(app.exec_())
